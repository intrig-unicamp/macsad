...
int read_macs_and_ports_from_file(char *filename) {
	...
	while (fgets(line, sizeof(line), f)) {
                ...
                ) )
		{
			...
			for( i = 0; i < 4; ++i )
					ipd[mac_count][i] = (uint8_t) values_ip[i];
				for( i = 0; i < 4; ++i )
				
				ipd_gre[mac_count][i] = (uint8_t) values_ip2[i];
	...
	void set_default_action_firewall_dl()
	{
		char buffer[2048]; /* TODO: ugly */
		struct p4_header* h;
		struct p4_set_default_action* sda;
		...
		sda = create_p4_set_default_action(buffer,0,sizeof(buffer));
		strcpy(sda->table_name, "firewall_dl");
	...
	void fill_nat_dw(uint8_t dtcp0, uint8_t ip[4])
	{
		char buffer[2048];
		struct p4_header* h;
		struct p4_add_table_entry* te;
	...
	void fill_firewall_dw(uint8_t dst_ip[4])
	{
		char buffer[2048];
		struct p4_header* h;
		struct p4_add_table_entry* te;
		...
		h = create_p4_header(buffer, 0, 2048);
		te = create_p4_add_table_entry(buffer,0,2048);
		strcpy(te->table_name, "firewall_dl");
	...
	void init() {
		...
		set_default_action_firewall_dl();
		...
		//--------- test 
		fill_nat_dw(dtcp,ip_dst_dw);   //  ips this case is ip_dst_dw:10.0.0.10
        //fill_firewall_dw(ip_dst_dw);
        fill_encap(ip_dst_dw,end_gre); //*DL
        //fill_firewall_dw(ip_dst_dw); 
        fill_ipv4_lpm_up(end_gre,port1,end_gre);
        ...
        for (i=0;i<=mac_count;++i)
        {
               ...
               //sleep(1);
               fill_nat_dw(dtcp_txt[i][1], ipd[i]);   //  ips this case is ip_dst_dw:10.0.0.10
               //fill_nat_dw(is_ext,ips[i],dtcp);   //  ips this case is ip_dst_dw:10.0.0.10
               //printf("inside sleep 0 \n");
               fill_encap(ipd[i],ipd_gre[i]);
               //printf("inside sleep 1 \n");
               fill_ipv4_lpm_up(ipd_gre[i],port1, ipd_gre[i]);
