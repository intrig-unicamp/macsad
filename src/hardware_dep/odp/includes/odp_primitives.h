...
#define FIELD_MASK(pd, field) (field_desc(pd, field).fixed_width ? field_desc(pd, field).mask : \
    odp_cpu_to_be_32((0xffffffff << (32 - FIELD_BITCOUNT(pd, field))) & (0xffffffff >> field_desc(pd, field).bitoffset)))
...
#define MODIFY_INT32_INT32_HTON(pd, dstfield, value32) { \
    if(field_desc(pd, dstfield).bytecount == 1) \
        ...
        res32 = (FIELD_BYTES(pd, dstfield) & ~FIELD_MASK(pd, dstfield)) | (odp_cpu_to_be_16(value32 << (16 - FIELD_BITCOUNT(pd, dstfield))) & FIELD_MASK(pd, dstfield)); \
    else \
        res32 = (FIELD_BYTES(pd, dstfield) & ~FIELD_MASK(pd, dstfield)) | (odp_cpu_to_be_32(value32 << (32 - FIELD_BITCOUNT(pd, dstfield))) & FIELD_MASK(pd, dstfield)); \
    memcpy(field_desc(pd, dstfield).byte_addr, &res32, field_desc(pd, dstfield).bytecount); \
}
...
#define GET_INT32_AUTO(pd, field) (field_desc(pd, field).meta ? \
    ...
    (odp_be_to_cpu_32(FIELD_MASKED_BYTES(pd, field)) >> (32 - FIELD_BITCOUNT(pd, field))))
...
#define EXTRACT_INT32_NTOH(pd, field, dst) { \
    if(field_desc(pd, field).bytecount == 1) \
        ...
        dst = odp_be_to_cpu_16(FIELD_MASKED_BYTES(pd, field)) >> (16 - FIELD_BITCOUNT(pd, field)); \
    else \
        dst = odp_be_to_cpu_32(FIELD_MASKED_BYTES(pd, field)) >> (32 - FIELD_BITCOUNT(pd, field)); \
}
