...
P4ODP_TARGET_DIR=${P4ODP_TARGET_DIR-"./build/$T4P4S_PRG"}
mkdir -p $P4ODP_TARGET_DIR
...
if [ "$T4P4S_P4" == 1 ]; then
    ...
    if [ "$PDB" != "" ]; then
        ...
        if echo "$PYVSN" | grep "Python 3[.][23456789]" >/dev/null; then
            echo $PYTHON -m "$PDB" -c continue src/transpiler.py "${P4_SOURCE}" --p4v $P4_VSN
            $PYTHON -c continue -m "$PDB" src/transpiler.py "${P4_SOURCE}" --p4v $P4_VSN
        else
            $PYTHON -m "$PDB" src/transpiler.py "${P4_SOURCE}" --p4v $P4_VSN
        fi
    else
        $PYTHON src/transpiler.py "${P4_SOURCE}" --p4v $P4_VSN
    fi
